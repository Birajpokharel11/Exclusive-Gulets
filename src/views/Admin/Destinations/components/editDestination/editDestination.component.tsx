...
import container from './editDestination.container';
...
function EditExperiences({
  ...
  useEffect(() => {
    if (router.query) {
      onFetchGenericOfferByIdStart(router.query.id);
    }
  }, [onFetchGenericOfferByIdStart, router]);
  ...
  useEffect(() => {
    ...
    setSideImage({
      preview: soleOffer.sideImage ?? null,
      raw: null
    });
  }, [soleOffer]);
